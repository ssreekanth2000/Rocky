...
float kd=0.475;// Movement constant
...
void updatePWMs(float totalDistanceLeft, float totalDistanceRight, float vL, float vR, float angleRad, float angleRadAccum) {
   ...
   */
  float dist= abs(totalDistanceLeft)*totalDistanceRight;
  p_d += -0.5*delta_t;
  //theta_d=-(totalDistanceLeft+totalDistanceRight)/2.0;
  theta_d=0.6*((totalDistanceLeft+totalDistanceRight/2.0)-p_d);//The desired theta to move the robot forward in the sprint
