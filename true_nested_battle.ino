...
float left_motor_PWM_offset = 0;
float right_motor_PWM_offset = 0;
...
void updatePWMs(float totalDistanceLeft, float totalDistanceRight, float vL, float vR, float angleRad, float angleRadAccum) {
  ...
  //theta_d=(-0.09*(vL+vR)/2)+(-0.9*(totalDistanceLeft+totalDistanceRight)/2);
  ...
  v_d=(kp*theta_err)+(ki*thet_int);

  vl_error=v_d-vL;
  vr_error=v_d-vR;
  
  vl_inte += vl_error*delta_t;
  vr_inte += vr_error*delta_t;


  //Serial1.println(theta_err);
  //Serial.println(leftMotorPWM);
  //Serial.print(v_d);
  leftMotorPWM = rp*vl_error+ri*vl_inte + left_motor_PWM_offset;
  rightMotorPWM = rp*vr_error+ri*vr_inte + right_motor_PWM_offset;
}
...
void loop()
{
  ...
  if (Serial1.available() > 0) {
    ...
    if (incomingByte == 65) { // user sent A (Left)
      left_motor_PWM_offset += 100;
    } else if (incomingByte == 68) { //user sent D (Right)
      right_motor_PWM_offset += 100;
    } else if (incomingByte == 87){ //user sent W (forward)
      left_motor_PWM_offset += 100;
      right_motor_PWM_offset += 100;
    } else if (incomingByte == 83){ //user sent S (back)
      left_motor_PWM_offset += -100;
      right_motor_PWM_offset += -100;
    }
