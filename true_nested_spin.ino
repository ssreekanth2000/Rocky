...
int kp = -3;//Proportianality constant for Angle PID
int ki = -36;//Integral constant for Angle PID
int rp =500;//Proportianality constant for Cruise control PID
int ri =5000;//Integral constant for Cruise control PID

float vl_error=0;//error in left wheel velocity
float vr_error=0;//error in Right wheel velocity
float vl_inte=0;//Integrated error in left wheel velocity
float vr_inte=0;//Integrated error in Right wheel velocity

float PWM;
float k_d=-0.8;//Proportionality constant to prevent runaway
float theta_d=0;//Desired theta
float theta_err=0;//Error in theta

float delta_t=0.01;//dt
void updatePWMs(float totalDistanceLeft, float totalDistanceRight, float vL, float vR, float angleRad, float angleRadAccum) {
   ...
   */
  //theta_d=(-0.09*(vL+vR)/2)+(-0.9*(totalDistanceLeft+totalDistanceRight)/2);//PI control for displacement which we abandoned as it didn't add any value
  theta_d=k_d*(totalDistanceLeft+totalDistanceRight)/2.0;// Setting theta_d to required angle to prevent runaway
  //theta_err=-0.110+theta_d-angleRad ;
  theta_err=theta_d - angleRad;//Error in theta
  
  thet_int+=theta_err*delta_t;//Integrated error in theta
  v_d=(kp*theta_err)+(ki*thet_int);//Desired velocity output from the angle proportional control

  vl_error=v_d-vL+0.3;// Error in left wheel velocity with offset added to make it spin
  vr_error=v_d-vR;
  
  vl_inte += vl_error*delta_t;//Integrated erro
  vr_inte += vr_error*delta_t;


  Serial.print(theta_err);
  //Serial.println(leftMotorPWM);
  //Serial.print(v_d);
  leftMotorPWM = (rp*vl_error+ri*vl_inte);//PWM output from cruise control PID
  rightMotorPWM = rp*vr_error+ri*vr_inte;
}
