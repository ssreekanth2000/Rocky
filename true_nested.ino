...
int kp = -3;//Angle PID Proportional
int ki = -36;//Angle PID Integral
int rp =500;//Velocity PID Proportional
int ri =5000;//Velocity PID Integral


float vl_error=0;//Velocity error
float vr_error=0;
float vl_inte=0;//Velocity error integrals
float vr_inte=0;

float k_d=1;//Dispalcement drift constant
float PWM;

float theta_d=0;//desired angle
float theta_err=0;//Theta error
float delta_t=0.01;
void updatePWMs(float totalDistanceLeft, float totalDistanceRight, float vL, float vR, float angleRad, float angleRadAccum) {
  ...
  theta_d=-k_d*(totalDistanceLeft+totalDistanceRight)/2.0;//Setting desired angle to make sure net displacemnt is zero
  //theta_err=-0.110+theta_d-angleRad ;
  theta_err=theta_d - angleRad;
  
  thet_int+=theta_err*delta_t;
  v_d=(kp*theta_err)+(ki*thet_int);//Angle PID

  vl_error=v_d-vL;//Calculating the error in velocity
  vr_error=v_d-vR;
  
  vl_inte += vl_error*delta_t;//Calculating the integral error in velocity
  ...
  //Serial.print(v_d);
  leftMotorPWM = rp*vl_error+ri*vl_inte;//Cruise control PID
...
void loop()
{
  ...
  if(angle > 5000 || angle < -5000)      // If angle is not within +- 3 degrees, reset counter that waits for start
  {
    start_counter = 0;
  }
...
/* Uncomment this and comment the above if doing wireless
  ...
  if(cur_time - prev_time > UPDATE_TIME_MS && (angle < -5000 || angle > 5000) && armed_flag)   
  {
    start_flag = 1;
    armed_flag = 0;
    angle_rad_accum = 0.0;
